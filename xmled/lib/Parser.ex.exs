...
defmodule XMLed.Parser do
  ...
  defcombinatorp :x_S,
    times(utf8_char([0x0020, 0x0009, 0x000D, 0x000A]), min: 1)
    |> label("S")
  ...
  defcombinatorp :x_NameStartChar,
    utf8_char([])
    |> label("NameStartChar")
  ...
  defcombinatorp :x_EntityValue, choice([
                    ignore(string("\"")) |> repeat(choice([ utf8_char([not: ?%, not: 38, not: ?"]), parsec(:x_PEReference), parsec(:x_Reference) ])) |> ignore(string("\"")),
                    ignore(string("'")) |> repeat(choice([ utf8_char([not: ?%, not: 38, not: ?']), parsec(:x_PEReference), parsec(:x_Reference) ])) |> ignore(string("'"))
                  ])
                |> tag(:entity_value)
                |> label("EntityValue")
  ...
  defcombinatorp :x_AttValue, choice([
                ignore(string("\"")) |> repeat(choice([ utf8_char([not: ?<, not: 38, not: ?"]), parsec(:x_Reference) ])) |> ignore(string("\"")),
                ignore(string("'")) |> repeat(choice([ utf8_char([not: ?<, not: 38, not: ?']), parsec(:x_Reference) ])) |> ignore(string("'"))
              ])
             |> tag(:att_value)
             |> label("AttValue")
  # SystemLiteral ::= ('"' [^"]* '"') | ("'" [^']* "'")
  defcombinatorp :x_SystemLiteral,
    choice([
  ...
  defcombinatorp :x_PubidChar, utf8_char([])
              |> label("PubidChar")
  ...
  defcombinatorp :x_PubidChar__WOQ, utf8_char([])
              |> label("PubidChar w/o quotes")

  # Character Data
  # CharData ::= [^<&]* - ([^<&]* ']]>' [^<&]*)
  defcombinatorp :x_CharData, repeat(lookahead_not(string("]]>") |> utf8_char([])))
             |> label("CharData")
  ...
  defcombinatorp :x_XMLDecl, ignore(string("<?xml"))
            ...
            |> concat(optional(parsec(:x_EncodingDecl)))
  ...
  defcombinatorp :x_intSubset, repeat(choice([parsec(:x_markupdecl), parsec(:x_DeclSep)]))
              |> label("intSubset")
  ...
  defcombinatorp :x_EncName,
    utf8_char([?A..?Z, ?a..?z])
    |> concat(repeat(utf8_char([?A..?Z, ?a..?z, ?0..?0, ?., ?_, ?-])))
    |> tag(:EncName)
    |> label("EncName")
  # NotationDecl ::= '<!NOTATION' S Name S (ExternalID | PublicID) S? '>'
  defcombinatorp :x_NotationDecl,
